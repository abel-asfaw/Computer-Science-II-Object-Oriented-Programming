...
void Game::MainMenu() {
  int foundableCount = 0;
  int menuChoice;
  while (menuChoice != QUIT and m_wizard.GetWins() != NUM_FOUNDABLE) {
...
void Game::PrintFoundables() {
  if (LoadFoundables() > 0) {
    ...
    cout << "Foundables have not been loaded from file.\n";
}
...
void Game::AttackFoundable() {
  ...
  if (defeated == false) {
	...
	if (m_allFoundables[i].GetRarity() == choice) {
      ...
      //tracks how many of a specific rarity wizard have defeated
      for (int i = 0; i < NUM_FOUNDABLE; i++)
	if (m_allFoundables[i].GetRarity() == choice) {
  ...
  //notify user if foundable has already been defeated
    ...
    //if all foundables of specic rarity have been defeated, notify user
    if (rarityCount == wizardRarityCount)
      cout << "***You've defeated all foundables of that rarity!***\n";
    else
